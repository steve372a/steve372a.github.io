...
qconfig_dependency(){
    echo "正在安装依赖...不要关闭终端！"
    sudo apt-get install libglib2.0-dev libfdt-dev libpixman-1-dev zlib1g-dev libsdl1.2-dev libsnappy-dev liblzo2-dev zlib1g-dbg automake gcc python -y
    sudo apt-get install build-essential -y
    make -j2
    make install
    echo "你的编译完成啦！点击“好的”测试一下编译是否成功！"
    echo "请先指明要测试的架构，然后再继续喔。"
    ...
    then echo "恭喜！编译完咯且运行正常！"
        exit
    else echo "似乎没有正确编译！你可以看看编译时出现了哪些错误，修补完后再继续。"
        exit
    fi
}
#设置configure
...
1. 是的
2. 不是"
read -r -p "请选择一个项目:" input_whostart
    case $input_whostart in
...
echo "检测运行环境..."
MACOS_MACHINE="$(uname)"
    if [[ $MACOS_MACHINE != "Linux" ]]; then
        echo "此脚本只能用于 Linux 环境！"
        exit
    fi
echo "${tty_green}是 Linux 哦，继续...${tty_reset}"
#检测 HomeBrew 安装情况
echo "检测 HomeBrew 安装情况..."
    if apt -v | grep -q APT; 
        then 
            echo "APT 存在。"
        else echo "必须是基于 Debian 的发行版。"
            exit      
    fi
qconfig_gitclone(){
    cd ~
...
qconfig_net(){
    ...
    if [ -e ~/qemu-${QEMU_VERSION}.tar.xz ]; then
        echo "将会直接解压包，因为你之前已经下载过。请点击“好的”继续。"
...
echo "菜单选项：(求白嫖呀制作 Shell script 脚本 for ${tty_blue}Linux(Debian)${tty_reset})
...
uninstall_clone_qemu_fd(){
    if [ -x ~/qemu/configure ]; then
        ...
        echo "我们没有在你的 PC 上面发现克隆镜像，不能继续操作！"
        exit
    fi
}
#更改下载包的地址
...
echo "默认下载版本："${tty_yellow}$QEMU_VERSION${tty_reset}
echo "本脚本版本："${tty_yellow}$QCONFIG_SHELL_VERSION${tty_reset}
echo "菜单选项：(求白嫖呀制作 for ${tty_blue}Linux(Debian)${tty_reset})
...
${tty_cyan}6. 卸载掉克隆的内容。
${tty_green}7. 查看更新内容${tty_reset}"
read -r -p "请选择一个项目:" input
    case $input in
        1)qconfig_gitclone;;
        2)qconfig_net;;
        3)qconfig_net_change;;
        4)echo "本程序版本为：0.2.2，编译日期为 2022 年 8 月 16 日。";;
        5)echo "求白嫖 的邮箱是：steve372@foxmail.com";;
        6)uninstall_clone_qemu_fd;;
        7)echo "请访问 https://steve372yeyeye.github.io/update/qcomplie";;
    esac